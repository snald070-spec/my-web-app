...
export default function InstallBanner() {
  ...
  useEffect(() => {
    ...
    if (isSamsungBrowser()) {
      // prompt() → WebAPK → Play Protect 차단
      // "홈 화면" 바로가기 방식은 WebAPK 없음 → 차단 없음
      ...
      <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)] max-w-sm">
        <div className="bg-gray-900 text-white rounded-2xl shadow-2xl px-4 py-4">
          ...
          <div className="bg-blue-800 rounded-xl px-3 py-2.5 mb-2">
            <p className="text-xs font-bold text-blue-200 mb-1">✅ 권장 (Play Protect 없음)</p>
            <p className="text-xs text-white leading-relaxed">
              <span className="font-semibold">삼성 인터넷</span> 앱에서 접속 →{' '}
              하단 <span className="font-semibold">⋮</span> → 페이지 추가 →{' '}
              <span className="font-semibold">홈 화면</span>
            </p>
          </div>
          <div className="flex items-center justify-between gap-2 mt-2">
            <p className="text-xs text-gray-400">크롬 직접 설치 (Play Protect 경고 가능)</p>
            <button
              onClick={async () => {
    ...
    return (
      <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)] max-w-sm">
        <div className="bg-gray-900 text-white rounded-2xl shadow-2xl px-4 py-3.5">
          ...
          <p className="text-xs text-gray-300 leading-relaxed">
            <span className="text-white font-semibold">삼성 인터넷</span>에서 접속 →{' '}
            하단 <span className="text-white font-semibold">⋮</span> → 페이지 추가 →{' '}
